...
(function() {
    ...
    const manualHTML = '<div class="manual-cover">' +
        ...
        '<li>Ou clique no menu do navegador (⋮) → "Instalar My Wallet..."</li>' +
        ...
        'João e Maria ganhavam R$ 5.000/mês. Sem controle, viviam no limite. Começaram a usar o My Wallet e aplicaram a regra 50-30-20:<br><br>' +
    ...
    class MyWallet {
        ...
        loadData() {
            try {
                const t = localStorage.getItem('Mywallet_transactions');
                if (t) this.transactions = JSON.parse(t);
                const c = localStorage.getItem('Mywallet_categories');
                if (c) this.categories = JSON.parse(c);
                else this.categories = JSON.parse(JSON.stringify(DEFAULT_CATEGORIES));
                const a = localStorage.getItem('Mywallet_accounts');
                if (a) this.accounts = JSON.parse(a);
                const cd = localStorage.getItem('Mywallet_cards');
                if (cd) this.cards = JSON.parse(cd);
                const inv = localStorage.getItem('Mywallet_investments');
                if (inv) this.investments = JSON.parse(inv);
                const dm = localStorage.getItem('Mywallet_dark');
                if (dm !== null) this.darkMode = dm === 'true';
                const pv = localStorage.getItem('Mywallet_privacy');
                if (pv !== null) this.privacyOn = pv === 'true';
            } catch (e) { console.error('[MyWallet] Erro ao carregar:', e); }
        }

        saveTransactions() { try { localStorage.setItem('Mywallet_transactions', JSON.stringify(this.transactions)); } catch(e){} }
        saveCategories() { try { localStorage.setItem('Mywallet_categories', JSON.stringify(this.categories)); } catch(e){} }
        saveAccounts() { try { localStorage.setItem('Mywallet_accounts', JSON.stringify(this.accounts)); } catch(e){} }
        saveCards() { try { localStorage.setItem('Mywallet_cards', JSON.stringify(this.cards)); } catch(e){} }
        saveInvestments() { try { localStorage.setItem('Mywallet_investments', JSON.stringify(this.investments)); } catch(e){} }
        ...
        saveFilters() {
            ...
            try { localStorage.setItem('Mywallet_filters', JSON.stringify(filters)); } catch(e){}
        }

        restoreFilters() {
            try {
                const filters = JSON.parse(localStorage.getItem('Mywallet_filters') || '{}');
        ...
        generateTimestamp() {
            ...
            return 'MyWallet-' + year + month + day + hours + minutes + seconds;
        }
        ...
        render() {
            ...
            sorted.forEach(t => {
                ...
                tr.onclick = function() { Mywallet.editTransaction(t.id); };
        ...
        applyTheme() {
            ...
            if (Object.keys(this.charts).length > 0) { try { this.updateChartsTheme(); } catch (e) { console.warn('[MyWallet] Erro tema:', e); } }
        }
        ...
        initCharts() {
            if (typeof Chart === 'undefined') { console.error('[MyWallet] Chart.js não carregado!'); return; }
            ...
            try { this.charts.line = new Chart(document.getElementById('lineChart').getContext('2d'), { type: 'line', data: { labels: [], datasets: [ { label: 'Receitas', data: [], borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', tension: 0.4 }, { label: 'Despesas', data: [], borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', tension: 0.4 } ] }, options: lineOpts }); } catch (e) { console.error('[MyWallet] Erro line:', e); }
            try { this.charts.pie = new Chart(document.getElementById('pieChart').getContext('2d'), { type: 'bar', data: { labels: [], datasets: [{ data: [], backgroundColor: [] }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true, ticks: { color: colors.textSecondary }, grid: { color: colors.grid } }, y: { ticks: { color: colors.textSecondary }, grid: { color: colors.grid } } }, barPercentage: 0.3, categoryPercentage: 0.5 } }); } catch (e) { console.error('[MyWallet] Erro pie:', e); }
            try { this.charts.cards = new Chart(document.getElementById('cardsChart').getContext('2d'), { type: 'line', data: { labels: [], datasets: [] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top', labels: { color: colors.text } } }, scales: { y: { beginAtZero: true, ticks: { color: colors.textSecondary }, grid: { color: colors.grid } }, x: { ticks: { color: colors.textSecondary }, grid: { color: colors.grid } } } } }); } catch (e) { console.error('[MyWallet] Erro cards:', e); }
            this.updateCharts();
        }

        updateChartsTheme() {
            const colors = this.getChartColors(); const self = this;
            Object.keys(this.charts).forEach(key => { const chart = self.charts[key]; if (!chart || !chart.options) return; try { if (chart.options.scales?.y?.ticks) chart.options.scales.y.ticks.color = colors.textSecondary; if (chart.options.scales?.y?.grid) chart.options.scales.y.grid.color = colors.grid; if (chart.options.scales?.x?.ticks) chart.options.scales.x.ticks.color = colors.textSecondary; if (chart.options.scales?.x?.grid) chart.options.scales.x.grid.color = colors.grid; if (chart.options.plugins?.legend?.labels) chart.options.plugins.legend.labels.color = colors.text; chart.update('none'); } catch (e) { console.warn('[MyWallet] Erro tema gráfico:', e); } });
        }
        ...
        renderCreditCardsList() {
            ...
            this.cards.forEach(card => {
                ...
                html += '<div class="cc-actions"><button class="cc-action-btn" onclick="event.stopPropagation(); Mywallet.editCard(\'' + card.id + '\')">✏️</button><button class="cc-action-btn" onclick="event.stopPropagation(); Mywallet.deleteCard(\'' + card.id + '\')">🗑️</button></div>';
        ...
        openInvoice(cardId) {
            ...
            html += '<button class="btn btn-secondary btn-small" onclick="Mywallet.exportInvoiceCSV(\'' + cardId + '\')">📥 CSV</button>';
            html += '<button class="btn btn-secondary btn-small" onclick="Mywallet.printInvoicePDF(\'' + cardId + '\')">🖨️ PDF</button></div></div>';
            html += '<div>';
            if (purchases.length === 0) { html += '<p style="text-align:center; padding:20px; color:var(--text-secondary);">Nenhuma compra neste período.</p>'; }
                ...
                purchases.sort((a,b) => new Date(a.date) - new Date(b.date)).forEach(p => {
                    ...
                    html += '<button class="btn btn-danger btn-small" onclick="Mywallet.deleteTransaction(' + p.id + '); Mywallet.openInvoice(\'' + cardId + '\')">🗑️</button></div>';
                });
            }
            html += '</div>';
            html += '<div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:20px;">';
            html += '<button class="btn btn-success" onclick="Mywallet.payInvoice(\'' + cardId + '\')">💰 Pagar Fatura</button>';
    ...
    window.dashboardAction = function(action) {
        ...
        switch(action) {
            ...
            case 'income': if (typeFilter) typeFilter.value = 'income'; if (statusFilter) statusFilter.value = ''; if (categoryFilter) categoryFilter.value = ''; if (accountFilter) accountFilter.value = ''; if (searchFilter) searchFilter.value = ''; Mywallet.clearCache(); Mywallet.render(); Mywallet.saveFilters(); document.querySelectorAll('.card.clickable')[1].classList.add('active-filter'); scrollToTransactions(); break;
            case 'expense': if (typeFilter) typeFilter.value = 'expense'; if (statusFilter) statusFilter.value = ''; if (categoryFilter) categoryFilter.value = ''; if (accountFilter) accountFilter.value = ''; if (searchFilter) searchFilter.value = ''; Mywallet.clearCache(); Mywallet.render(); Mywallet.saveFilters(); document.querySelectorAll('.card.clickable')[2].classList.add('active-filter'); scrollToTransactions(); break;
    ...
    window.changeCardMonth = function(delta) { Mywallet.cardModalMonth.setMonth(Mywallet.cardModalMonth.getMonth() + delta); Mywallet.renderCreditCardsList(); };
    window.changeCardMonthToToday = function() { Mywallet.cardModalMonth = new Date(); Mywallet.cardModalMonth.setDate(1); Mywallet.renderCreditCardsList(); };
    window.updateCardMonthLabel = function() { const months = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']; const label = document.getElementById('cardMonthLabel'); if (label) label.textContent = months[Mywallet.cardModalMonth.getMonth()] + ' ' + Mywallet.cardModalMonth.getFullYear(); };

    window.openNewTransactionModal = function() { Mywallet.setDefaultDate(); Mywallet.currentTransactionType = 'expense'; document.querySelectorAll('#transactionForm .type-btn').forEach(btn => { const isExpense = btn.getAttribute('data-type') === 'expense'; btn.classList.toggle('active', isExpense); btn.setAttribute('aria-checked', isExpense); }); Mywallet.populateCategorySelects(); Mywallet.populatePaymentMethodSelects(); Mywallet.populateAccountSelects(); document.getElementById('newTransactionModal').classList.add('active'); };
    window.closeNewTransactionModal = function() { document.getElementById('newTransactionModal').classList.remove('active'); Mywallet.clearForm(); };
    window.closeEditModal = function() { document.getElementById('editModal').classList.remove('active'); Mywallet.currentEditId = null; };
    ...
    window.openAccountsModal = function() { Mywallet.renderAccountsList(); document.getElementById('accountsModal').classList.add('active'); document.getElementById('mainMenu').classList.remove('active'); };
    ...
    window.openCreditCardsModal = function() { Mywallet.cardModalMonth = new Date(Mywallet.currentMonth); Mywallet.renderCreditCardsList(); document.getElementById('creditCardsModal').classList.add('active'); document.getElementById('mainMenu').classList.remove('active'); };
    ...
    window.printManual = function() { Mywallet.printManual(); };
    ...
    window.togglePrivacy = function() { Mywallet.privacyOn = !Mywallet.privacyOn; localStorage.setItem('Mywallet_privacy', Mywallet.privacyOn); Mywallet.applyPrivacy(); };
    window.toggleTheme = function() { Mywallet.darkMode = !Mywallet.darkMode; localStorage.setItem('Mywallet_dark', Mywallet.darkMode); Mywallet.applyTheme(); };
    ...
    window.copyPixKey = function() { const key = document.getElementById('pixKey').textContent; navigator.clipboard.writeText(key).then(() => Mywallet.showToast('✅ Chave PIX copiada!')).catch(() => Mywallet.showToast('❌ Copie manualmente: ' + key)); };
    ...
    if ('serviceWorker' in navigator) { window.addEventListener('load', () => { navigator.serviceWorker.register('sw.js').then(reg => console.log('[MyWallet] SW registrado:', reg.scope)).catch(err => console.log('[MyWallet] SW falhou:', err)); }); }

    console.log('🎉 My Wallet v4.0.3 carregado com sucesso!');
})();
